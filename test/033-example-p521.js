...
var suite = Suite.create(mocha.suite,'Running the example - p521');
...
co(function* () {
  ...
  suite.addTest(new Test('Loading parties', function(done) {
    scope.alice   = new scope.EC('p521');
    scope.bob     = new scope.EC('p521');
    scope.charlie = new scope.EC('p521');
  ...
  suite.addTest(new Test('Key generation', function(done) {
    ...
    assert.equal(scope.alice.kp.pri.data.length,65);
    assert.equal(scope.bob.kp.pri.data.length,65);
    assert.equal(scope.charlie.kp.pri.data.length,65);
    done();
  }));
