...
EC.prototype.sign = function( data, key ) {
  var pri;
  key = key || this.kp;
  if ( key instanceof KeyPair ) pri = key.pri.data;
...
EC.prototype.verify = function( data, signature, key ) {
  var pub;
  key = key || this.kp;
  if ( key instanceof KeyPair ) pub = key.pub.data;
  if ( Buffer.isBuffer(key) ) pub = key;
  if ( 'string' === typeof key ) pub = Buffer.from(key,'base64');
  if (!('string' === typeof data) && !Buffer.isBuffer(data)) data = JSON.stringify(data);
  var msg = this.H(data),
      sig = Buffer.from(signature,keyservice.format);
  return this.ec.verify(msg,{r:sig.slice(0,32),s:sig.slice(32,64)},key);
};
